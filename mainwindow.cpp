//Main window where graph and tables are presented from microdevice.
//Tools get connected from mainwindow and launch popups when selected values are valid
//
    ...
    ui(new Ui::MainWindow)
{
    ...
    QFile file(filePath); // *this needs to be changed to be a more mobile handle, for now, keep it local*
    ...
    // create multiple test graphs with random generated values:
...
void MainWindow::myfunction() //counter code thats called up when watchdog is triggered, countdown stops when code crashes
{
    num = num + 1;
    if (num > 100)
        num = 0;
...
void MainWindow::markerfc(QStringList popupData){
    //Marker tool function, selects value from graph and initalizes popup. Once value from popup is returned, graph the marker
...
void MainWindow::labelfc(QStringList popupData){
    //Label tool function, selects value from graph and initalizes popup. Once value from popup is returned, graph the label
...
void MainWindow::on_toolButton_2_clicked() //LABEL TOOL BUTTON
{
...
void MainWindow::on_toolButton_3_clicked() //HAND TOOL BUTTON
{
